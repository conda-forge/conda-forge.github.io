...
cxx_compiler_version:   # [osx]
  - 15                  # [osx]
```
in `conda_build_config.yaml` and continue to use the well-established `{{ compiler("c") }}` and `{{ compiler("cxx") }}` macros in the recipe;
be aware that you're then responsible for updating the version roughly once per year (to keep pace with the rest of conda-forge).
