...
man_pages = [(master_doc, "conda-forge", "conda-forge Documentation", [author], 1)]

# If true, show URL addresses after external links.
# man_show_urls = False


# -- Options for Pydantic Autodoc output -------------------------------------------
...
def rstjinja(app, docname, source):
    def get_formated_names(path_file):
        ...
        return "\n".join(
            f"* `{m['name']},"
            f" @{m['github_username']}"
            f" <https://github.com/{m['github_username']}>`__"
            for m in sorted_csv
        )

    if app.builder.format != "html" or os.path.basename(docname) != "governance":
        return
    src = source[0]
    current_file = os.path.dirname(__file__)
    context = app.config.html_context
    context["core_members"] = get_formated_names(os.path.join(current_file, "core.csv"))
