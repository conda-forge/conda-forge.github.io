...
var _Http_toTask = F3(function(router, toTask, request)
{
	return _Scheduler_binding(function(callback)
	{
		...
		try {
			xhr.open(request.h, request.a1, true);
		} catch (e) {
...
var elm$http$Http$request = function (r) {
	return elm$http$Http$command(
		elm$http$Http$Request(
			{x: false, aw: r.aw, aB: r.aB, e: r.e, h: r.h, i: r.i, j: r.j, a1: r.a1}));
};
var elm$http$Http$get = function (r) {
	return elm$http$Http$request(
		{aw: elm$http$Http$emptyBody, aB: r.aB, e: _List_Nil, h: 'GET', i: elm$core$Maybe$Nothing, j: elm$core$Maybe$Nothing, a1: r.a1});
};
		...
		return elm$http$Http$get(
			{
				...
				a1: 'https://libcflib.conda-forge.org/search?query=' + (query + ('&page_num=' + elm$core$String$fromInt(page_num)))
			});
	});
		...
		switch (msg.$) {
			...
			case 1:
				return _Utils_Tuple2(
					...
					A2(author$project$Main$getQuery, model.aQ, model.aM));
			case 2:
				var value = msg.a;
				return _Utils_Tuple2(
					_Utils_update(
						model,
						{r: elm$core$Maybe$Nothing, aM: value, v: elm$core$Maybe$Nothing}),
					A2(author$project$Main$getQuery, model.aQ, value));
			case 3:
				var field = msg.a;
				var value = msg.b;
				return _Utils_Tuple2(
...
var author$project$Main$Query = 0;
	...
	return A2(
		...
		_List_fromArray(
			[
				...
				author$project$Main$onEnter(author$project$Main$SubmitForm)
			]),
		_List_fromArray(
			[
				A2(
	...
	return A2(
		...
		_List_fromArray(
			[
				...
				_List_fromArray(
					[
						elm$html$Html$text('Search Errors')
					])),
				A2(
				elm$html$Html$div,
				_List_Nil,
			...
			[
				...
				_List_fromArray(
					[
						...
						_List_fromArray(
							[
								elm$html$Html$Attributes$href('/artifact.html?pkg=' + (artifact.aX.aP + ('&channel=' + (artifact.aX.ay + ('&arch=' + (artifact.aX.au + ('&name=' + artifact.aX.ac)))))))
							]),
						_List_fromArray(
							[
								elm$html$Html$text(artifact.ac + (' v' + artifact.a2))
							]))
					])),
				...
				_List_fromArray(
					[
						elm$html$Html$text(artifact.aX.aO)
					]))
			]));
};
var author$project$Main$UpdatePageNum = function (a) {
	return {$: 2, a: a};
};
var author$project$Main$viewPageBarLink = function (page_num) {
	return A2(
		elm$html$Html$button,
		_List_fromArray(
			[
				elm$html$Html$Events$onClick(
				author$project$Main$UpdatePageNum(page_num))
			]),
		_List_fromArray(
			[
				elm$html$Html$text(
				elm$core$String$fromInt(page_num))
			]));
};
...
var author$project$Main$viewPageBar = function (page_num) {
	return A2(
		elm$html$Html$div,
		_List_fromArray(
			[
				elm$html$Html$Attributes$class('page-bar')
			]),
		elm$core$List$concat(
			_List_fromArray(
				[
					(page_num === 1) ? _List_fromArray(
					[
						elm$html$Html$text('')
					]) : A2(
					elm$core$List$map,
					author$project$Main$viewPageBarLink,
					A2(
						elm$core$List$range,
						A2(elm$core$Basics$max, page_num - 5, 1),
						page_num - 1)),
					_List_fromArray(
					[
						elm$html$Html$text(
					...
					A2(
						elm$core$List$range,
						page_num + 1,
						A2(elm$core$Basics$max, page_num + 5, 10)))
				])));
};
	...
	return A2(
		...
		_List_fromArray(
			[
				...
				_List_fromArray(
					[
						elm$html$Html$text('searched: \'' + (response.aQ + '\''))
					])),
				author$project$Main$viewPageBar(response.aM),
				A2(
				elm$html$Html$ol,
				_List_fromArray(
					[
						elm$html$Html$Attributes$start(((response.aM - 1) * response.aN) + 1)
					]),
				elm$core$List$map(author$project$Main$viewArtifact)(response.aU)),
				author$project$Main$viewPageBar(response.aM)
			]));
};
var author$project$Main$viewUtils = F2(
	function (model, viewF) {
		return A2(
			elm$html$Html$div,
			_List_Nil,
			_List_fromArray(
...
var author$project$Main$view = function (model) {
	return A2(author$project$Main$viewUtils, model, author$project$Main$viewForm);
};
...
var elm$url$Url$Url = F6(
	function (protocol, host, port_, path, query, fragment) {
		return {Z: fragment, _: host, aO: path, af: port_, ai: protocol, aQ: query};
	});
