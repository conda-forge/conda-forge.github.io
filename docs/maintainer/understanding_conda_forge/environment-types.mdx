...
details see [compilation concepts](../compilation-concepts)), we are forced to be very precise
...
indicated by the presence of two sections annotated by the `[build time]` tag. The `build:` environment is where we place anything that needs
...
binaries compiled for `linux-aarch64` cannot run on our `linux-64` build machine.

Finally, the `run:` environment does not have any role at build time. It only specifies
what dependencies need to be present for `mypkg` to be functional once installed. In many cases, libraries from
