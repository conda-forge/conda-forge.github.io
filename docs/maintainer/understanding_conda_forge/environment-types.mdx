...
For example, our CI providers do not offer native `linux-aarch64` machines at scale for free, and so
we are forced to cross-compile packages for this platform from `linux-64`. That said, it is possible
...
or `linux-ppc64le`; for more details see [compilation concepts](../compilation-concepts)), so we are
forced to be very precise about separating the necessary ingredients for building a package.
...
may be rebuilt more often than strictly necessary. You should always address those warnings.
...
expects to find the library), you can try ignoring its run-exports:
