...
Although the topic of [cross-compilation](../cross-compilation) is somewhat of an advanced topic, the constraints
...
we will use to build those packages (sidenote: due to limitations of the YAML format, hyphens have
to be replaced with underscores; we will continue using `linux-aarch64` etc. below).

In general, packages compiled for one architecture can only run on that architecture (e.g. a package
built for linux-aarch64 can only be executed in that type of machine, but not `linux-64` or
`linux-ppc64le`; for more details see [compilation concepts](../compilation-concepts)), we are forced
to be very precise about separating the necessary ingredients for building a package.

Let's take a simple recipe for `mypkg` and annotate what happens when we build it for `linux-aarch64` on a `linux-64` machine (`host_platform: linux-aarch64`, `build_platform: linux-64`).
...
_present_); other than that it is a regular conda environment, whose path is reachable during the
...
symbols when linking. With the very few exceptions, things in `host:` cannot be executed during the
...
Finally, the `run:` environment does not have any role at build time. It only specifies which
...
ABI-tracking in `host:` is **not transitive**: `foo` tracks `bar` but not `baz`.

This is why the link check will warn you if you are "overdepending" on libraries: you should not
declare `host:` dependencies that your package doesn't use directly (though be aware that
