...
#### v1 recipe (recommended)

Whenever possible, you will want to start with an automatically generated recipe using
[rattler-build generate-recipe command](https://rattler-build.prefix.dev/latest/recipe_generation/).
At the time of writing, the tool supports generating recipes for:

- Python packages from PyPI
- R packages from CRAN
- Perl packages from CPAN
- Lua packages from LuaRocks

For a full list, consult the output of `rattler-build generate-recipe`. Note that the generated
recipe needs to be reviewed and modified as necessary.

If no generator is available for the package type in question, start with
[the example recipe](https://github.com/conda-forge/staged-recipes/tree/main/recipes/example-v1)
in the [staged-recipes repository](https://github.com/conda-forge/staged-recipes) and modify it.
Your final recipe should have no comments (unless they're actually relevant to the recipe, and not
generic instruction comments), and follow the order in the example.
...
There are, currently, three ways to generate a v0 recipe:
...
7. Modify the copied recipe file as needed. To see how to modify it, take a look at
   [The recipe (recipe.yaml or meta.yaml)](#the-recipe-recipeyaml-or-metayaml).
...
Both files use the [yaml](https://en.wikipedia.org/wiki/YAML) format, augmented with [Jinja](http://jinja.pocoo.org/) templating.

The full reference of the formats can be found in the following sources:
- `recipe.yaml`: [rattler-build: the recipe spec](https://rattler-build.prefix.dev/latest/reference/recipe_file/)
...
a selector expression via templating.
...
Existence testing can be accomplished using `script` tests in v1 recipes, or `commands` subsection
of the `test` section of v0 recipe.
...
tests:
  - script:
      - if: unix
        then: test -f $PREFIX/lib/libboost_log$SHLIB_EXT
      - if: win
        then: if not exist %LIBRARY_LIB%\\boost_log-vc140-mt.lib exit 1
```
...
test:
  ...
  commands:
    - python -m pytest tests test_pkg_integration.py
```
...
`files.source` or `source_files` keys to copy the tests for the
testing phase only.
...
test:
  commands:
    - $R -e "testthat::test_dir('tests/testthat/', package='PackageName', load_package='installed', stop_on_failure=TRUE)"           # [not win]
    - "\"%R%\" -e \"testthat::test_dir('tests/testthat/', package='PackageName', load_package='installed', stop_on_failure=TRUE)\""  # [win]
```
