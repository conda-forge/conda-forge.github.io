...
- Step 1. Staging process (add recipe and license).
  ...
  With the help of [the staging process](#the-staging-process), add a package's recipe and license to the [staged-recipes repository](https://github.com/conda-forge/staged-recipes) and create a PR.
...
1. Ensure your source code can be downloaded as a single file. Source code
   ...
   detail, see [Build from tarballs, not repos](#build-from-tarballs-not-repos)).
...
6. [Generate the recipe](#generating-the-recipe) or copy the example `recipe.yaml` (or `meta.yaml`)
   into this folder.
   All the changes in the following steps will happen in the COPIED `recipe.yaml` or `meta.yaml`
   (i.e., `...staged-recipes/recipes/<name-of-package>/recipe.yaml`).
   Please leave the example directory unchanged!
7. Modify the copied recipe file as needed. To see how to modify it, take a look at
   [The recipe (recipe.yaml or meta.yaml)](#the-recipe-recipeyaml-or-metayaml) section.
8. Generate the SHA256 hash for your source code archive, as described in the
...
9. Be sure to fill in the `test` section. The simplest test will simply
   ...
   some other modules that do contain runnable code.
...
- Ensure that the license has the right case and that the license is correct. Note that case sensitive inputs are required (e.g. `Apache-2.0` rather than `APACHE 2.0`). Using SPDX identifiers for license field is recommended. (see [SPDX Identifiers and Expressions](#spdx-identifiers-and-expressions))
- Ensure that you have included a license file if your license requires one – most do. (see [v0 example](https://github.com/conda-forge/staged-recipes/blob/f12772fa1d8c71c1028c2a41ef41c8278520ae5b/recipes/example/meta.yaml#L84-L88) or [v1 example](https://github.com/conda-forge/staged-recipes/blob/f12772fa1d8c71c1028c2a41ef41c8278520ae5b/recipes/example-v1/recipe.yaml#L88-L92))
...
- Recommended: run the test locally on your source code to ensure the recipe works locally (see [Running tests locally for staged recipes](#running-tests-locally-for-staged-recipes)).
...
Once these steps are complete, you can continue with the steps in [Step-by-step Instructions](#step-by-step-instructions) to stage your new package recipe using your existing staged-recipes fork.
...
The recipe file is at the heart of every conda package. It is located in the `recipe` directory,
and is named either:
...
Both files use [YAML](https://en.wikipedia.org/wiki/YAML) syntax, augmented with [Jinja](http://jinja.pocoo.org/) templating.

The full reference of the formats can be found in the following sources:
- `recipe.yaml`: improved recipe format as specified by [CEP 13](https://github.com/conda/ceps/blob/main/cep-0013.md) & [CEP 14](https://github.com/conda/ceps/blob/main/cep-0014.md). As of early 2026, it is only implemented by rattler-build, which also provides a more readable [overview](https://rattler-build.prefix.dev/latest/reference/recipe_file).
- `meta.yaml`: [conda-build: defining metadata (meta.yaml)](https://conda.io/projects/conda-build/en/stable/resources/define-metadata.html)

In the following, we highlight particularly important and conda-forge specific information and
...
`conda-build` (v3 and above) and `rattler-build` support multiple sources per recipe. Examples are available in the [rattler-build](https://rattler-build.prefix.dev/latest/reference/recipe_file/#source-section) and [conda-build docs](https://docs.conda.io/projects/conda-build/en/stable/resources/define-metadata.html#source-from-multiple-sources).
...
- a list of selector expressions, in which case the build is skipped if any of the expressions
  evaluates to true.

In v0 recipes, only a boolean value is directly accepted, and it needs to be combined with
a selector expression via templating. Different selectors can either be combined using `or`
clauses or multiple `skip: true` can be used.
...
A full description of selectors is in the [rattler-build](https://rattler-build.prefix.dev/latest/selectors/) and
[conda-build docs](https://docs.conda.io/projects/conda-build/en/stable/resources/define-metadata.html#preprocessing-selectors).

<a id="optional-bld-bat-and-or-build-sh"></a>

#### Optional: build scripts

In many cases, explicit build scripts are not required.
...
as the installation script in the recipe file or the build script files,
...
Three primary kinds of dependencies are recognized.
In the following paragraphs, we give a very short overview what packages go where.
For a detailed explanation please refer to the [rattler-build](https://rattler-build.prefix.dev/latest/reference/recipe_file/#requirements-section) or [conda-build documentation](https://docs.conda.io/projects/conda-build/en/stable/resources/define-metadata.html#requirements-section), respectively.
...
- compilers (see [Compilers](../knowledge_base#compilers))
- cmake
- make
- pkg-config
- CDT packages (see [Core Dependency Tree Packages (CDTs)](../knowledge_base#core-dependency-tree-packages-cdts))
...
- python/r libraries that link against c libraries (see e.g. [Building Against NumPy](../knowledge_base#building-against-numpy))
- python, r-base
...
1. Some dependencies have to be satisfied with [CDT](../glossary.md#cdt) packages (see [Core Dependency Tree Packages (CDTs)](../knowledge_base#core-dependency-tree-packages-cdts)).
...
- exports its [ABI](../glossary.md#abi) compatible requirements by defining `run_exports` in its recipe file
...
In this case run constraints can be used to restrict `awesome-software` to version 1.\*, if the user
chooses to install it. In v1 recipes, the key `run_constraints` is used, whereas in v0 recipes it's
`run_contrained`:
...
In combination with an unsatisfiable version, run constraints can define blockers:
...
In this example, `awesome-db` cannot be installed together with `amazing-db` as the constraint
`amazing-db <0.0a0` is impossible to satisfy.
...
In v1 recipes, existence testing can be accomplished `package_contents` tests. In v0 recipes, it
needs to be done manually in the `commands` subsection of the `test` section.

On POSIX systems, use the `test` utility and the `$PREFIX` variable. On Windows, use the `exist`
...
test:
  commands:
    ...
    - if not exist %LIBRARY_BIN%\\boost_log.dll exit 1  # [win]
    - if not exist %LIBRARY_LIB%\\boost_log.lib exit 1  # [win]
```
...
For the best information about testing, see the test sectios in the
[rattler-build](https://rattler-build.prefix.dev/latest/reference/recipe_file/#tests-section)
or [conda-build docs](https://docs.conda.io/projects/conda-build/en/stable/resources/define-metadata.html#test-section).
...
In v1 recipes, this is accomplished using a `python` test which combines both `imports` testing
and an automatic `pip check` test. In v0 recipes, a combination of `imports` test with an explicit
command test needs to be used:
...
not necessarily the name of the conda package (they are sometimes different), or even the wheel name.
...
`pytest`. You can ensure that it is included by adding it to requirements
in the test stanza:
...
You can include files required for testing with the `files/source` section in v1 recipes,
or `source_files` section in v0 recipes:
...
Alternatively, a custom test script can be created and placed in the `recipe`
directory. This allows an arbitrarily complicated test script. For v1 recipes,
the script is listed via a `script` test with a `file` key:
...
For v0 recipes, the file is always called `run_test.py`, and it is used
automatically when present. However, note that it will override the `test:`
section of the recipe entirely and cause it to be silently ignored.
...
can be successfully imported. This is accomplished with:
...
`files.source` or `source_files` keys (for `recipe.yaml` or `meta.yaml`, respectively) to copy the tests for the
testing phase only.
...
If not specified, the name will be taken from the top-level `name` field in the recipe file.
