...
```recipe title="recipe/meta.yaml (non-noarch)"
name: package
source:
  # ...
build:
...
```recipe title="recipe/recipe.yaml (non-noarch)"
name: package
source:
  # ...
build:
...
The solver requires virtual packages to be present in the package metadata in order to
select the correct package for your platform.
...
builds: only the Windows build will include a virtual package (`__win`), and the other
