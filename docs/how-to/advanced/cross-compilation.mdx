...
conda-build input metadata. The [test](/docs/maintainer/conda_forge_yml/#test) key can be used to skip the test phase when
...
- If the package provides binaries that need to be run during the build process, it goes into
  `build`. Examples include the compiler, `make`, `meson`, `pkg-config`, `sed` and so on.
- If the package provides libraries or headers that are used to build the installed binaries or the
  test suite, it goes into `host`. Examples include `eigen`, `libxml2-devel`, `zlib` and so on. For
  historical reasons, `python` also belongs in `host` dependencies, but see [Python
  cross-compilation](#details-about-cross-compiled-python-packages).
- If both conditions are true, the package belongs in both sections (in the `build` section, it may
  need to be made conditional to cross-compiling). An example of such a package is `llvmdev`.
...
In some cases, additional packages may be needed only when cross-compiling. To cover that, you can
...
However, there are some cases requiring special handling; most notably [Python cross-compilation](#details-about-cross-compiled-python-packages).

## Testing

Running the test suites of the packages generally requires executing binaries built for the host
...
If the configure scripts needs to run programs in order to determine the system features, it will
...
This example is discussed in greater detail in [details about cross-compiled Python
packages](#details-about-cross-compiled-python-packages).
...
Cross-compiling R packages, similar to Python, requires a specialized `cross-r-base` and R library dependencies in the `build` environment.
A simple R package requiring C compilation and depending on `r-matrix` would look something like:
...
The example above omits typical overhead needed to support Windows compilation, which relies on [the MinGW-based compiler stack](/docs/maintainer/infrastructure/#mingw-based-compiler-stack-for-windows) for nearly all R recipes. See [the `r-mgcv` feedstock](https://github.com/conda-forge/r-mgcv-feedstock) for a full example.
...
1. Add `cross-python_${{ host_platform }}` (or `cross-python_{{ target_platform }}` for v0 recipes)
   to `build` requirements, conditionally to the cross-compiling selector.
2. Copy `python` itself and non-pure Python packages (i.e. these that ship compiled extensions)
   that need to be present while the package is being built, such as `cython` and `numpy`, from
   `host` to `build` requirements, conditionally to the cross-compiling selector.

This is demonstrated in the [Python example](#python).
