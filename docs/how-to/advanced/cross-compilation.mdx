...
- If the package provides libraries or headers that are used to build the installed binaries, it
  goes into `host`. Examples include `eigen`, `libxml2-devel`, `zlib` and so on. For historical
  reasons, `python` also belongs in `host` dependencies, but see [Python
