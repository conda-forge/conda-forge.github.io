...
Python packages can also have "external" dependencies. The standardized approach to declaring them
is currently worked on as [PEP 725](https://peps.python.org/pep-0725/). These packages sometimes
use general purpose build systems declared in the earlier sections, and their external dependencies
can be obtained from them. In some cases, these dependencies are additionally packaged on PyPI (for
example, `cmake` and `ninja` are).
