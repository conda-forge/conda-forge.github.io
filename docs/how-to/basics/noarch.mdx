...
All recipes employing `noarch: python` must use the `python_min` variable per the following example:
...
1. Adding an explicit `python_min` to your `noarch: python` recipe can be an effective way to ensure the required Python in your package's metadata is enforced at `conda-build` time, as the build will fail if the package's required Python version is newer than `python_min`.
2. While `noarch: python` does not work with selectors, it does work with version constraints.
   `skip: match(python, "<3.12")` can be replaced with `python_min: 3.12` override.
3. Only `console_scripts` entry points have to be listed in your recipe explicitly, as they require special treatment to create platform-appropriate scripts.
   Other types of entry points do not conflict with `noarch` and therefore do not require extra treatment.
4. `noarch` is a statement about the package's source code and not its install environment.
...
5. If an existing python package qualifies to be converted to a noarch package, you can request the required changes by opening a new issue and including `@conda-forge-admin, please add noarch: python`.
...
feedstock rerender to be applied. See [Rerendering feedstocks](rerender.md).
